...
export class PassportComponent {
  ...
  toggleMode() { this.isRegister = !this.isRegister; this.error = ''; this.success = ''; }

  loginWithGoogle() {
    this.error = ''; this.success = '';
    this.auth.loginWithGoogle()
      .then(() => {
        this.msg.show({ severity: 'success', summary: '登入成功', detail: 'Google 登入成功' });
  ...
  loginWithEmail() {
    ...
    this.auth.loginWithEmail(this.email, this.password)
      .then(() => {
        this.msg.show({ severity: 'success', summary: '登入成功', detail: 'Email 登入成功' });
        this.router.navigate(['/contract']);
      })
      .catch((e: Error) => this.error = e.message);
  }
